...
const Explore: React.FC = () => {
  ...
  const displayedArtworks = useMemo(() => {
    ...
    // Helper to convert category labels (e.g. "Digital Art") to slug format ("digital-art")
    // Convert category labels to kebab-case slugs (e.g. "Digital Art" → "digital-art")
  ...
  return String(str).toLowerCase().trim().replace(/\s+/g, '-');
    ...
    return allArtworks.filter(artwork => {
      ...
      if (selectedCategory !== 'all') {
        const artworkCatSlug = slugify(artwork.category || '');
        if (artworkCatSlug !== selectedCat) {
          return false;
        }
      }
